...
/// Source of a file digest.
#[derive(Clone, serde::Serialize)]
#[serde(rename_all = "snake_case")]
pub enum DigestSource {
    /// Extracted from document structure (headings + paragraphs).
    Text,
    /// Generated by an AI provider.
    Ai,
}

/// A file digest with its source.
#[derive(Clone, serde::Serialize)]
pub struct Digest {
    /// The digest text.
    pub text: String,
...
pub async fn get_file_preview_info(
    ...
    let (title, digest) = match DocumentType::from_path(path_buf) {
        Some(DocumentType::Markdown) => {
            ...
            let digest = if let Some(text) = ai_digest {
                Some(Digest {
                    text,
                    source: DigestSource::Ai,
                })
            } else {
                content
                    .as_ref()
                    .and_then(|c| extract_digest(c, 12, 1000))
                    .map(|text| Digest {
                        text,
                        source: DigestSource::Text,
                    })
            };
            (title, digest)
        }
        Some(DocumentType::Pdf) => (
            get_pdf_title(path_buf),
            ai_digest.map(|text| Digest {
                text,
                source: DigestSource::Ai,
            }),
        ),
