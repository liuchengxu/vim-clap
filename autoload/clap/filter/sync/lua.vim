...
if has('nvim-0.5')
  ...
  function! clap#filter#sync#lua#(query, candidates, _winwidth, enable_icon, matching_text_kind) abort
    let g:_clap_lua_query = a:query
    let g:_clap_lua_candidates = a:candidates
    let g:_clap_lua_enable_icon = a:enable_icon
    let g:_clap_lua_matching_text_kind = a:matching_text_kind

lua << EOF
local fzy_filter = require('fzy_filter')
vim.g.__clap_fuzzy_matched_indices, vim.g._clap_lua_filtered =
    fzy_filter.do_fuzzy_match(vim.g._clap_lua_query, vim.g._clap_lua_candidates, vim.g._clap_lua_enable_icon, vim.g._clap_lua_matching_text_kind)
EOF
...
matched_indices, _clap_lua_filtered =
    fzy_filter.do_fuzzy_match(vim.eval('a:query'), lines, vim.eval('a:enable_icon'), vim.eval('a:matching_text_kind'))
