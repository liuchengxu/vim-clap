...
impl SessionManager {
    ...
    pub fn new_session(&mut self, session_id: u64, msg: Message) {
        match session::spawn_new_session(msg) {
            Ok(sender) => {
    ...
    /// Send Terminate event to stop the session thread.
...
pub fn spawn_new_session(msg: Message) -> anyhow::Result<crossbeam_channel::Sender<SessionEvent>> {
