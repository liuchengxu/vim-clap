...
use types::{LinesTruncatedMap, SourceItem, FilterResult};
...
) -> (Vec<(SourceItem, T, Vec<usize>)>, LinesTruncatedMap) {
        ...
        .map(|(item, score, indices)| {
            ...
            if !indices.is_empty() {
                ...
                if *last_idx > winwidth {
                    ...
                    let end = item.display.len();
                    let truncated = if let Some(n) = skipped {
                        let icon: String = item.display.chars().take(n).collect();
                        start += n;
                        format!("{}{}{}", icon, DOTS, utf8_str_slice(&item.display, start, end))
                    } else {
                        format!("{}{}", DOTS, utf8_str_slice(&item.display, start, end))
                    };
                    let offset = line_len - truncated.len();
                    let truncated_indices = indices.iter().map(|x| x - offset).collect::<Vec<_>>();
                    truncated_map.insert(lnum, item.display);
                    (SourceItem::new(truncated), score, truncated_indices)
                } else {
                    (item, score, indices)
                }
            } else {
...
pub fn process_top_items<T>(
    ...
    if let Some(painter) = icon_painter {
        for (idx, (item, _, idxs)) in truncated_lines.iter().enumerate() {
            let iconized = if let Some(origin_text) = truncated_map.get(&(idx + 1)) {
                format!("{} {}", painter.get_icon(origin_text), item.display)
            } else {
                painter.paint(&item.display)
            };
            lines.push(iconized);
            indices.push(idxs.iter().map(|x| x + ICON_LEN).collect());
        }
    } else {
        for (item, _, idxs) in truncated_lines {
...
pub fn print_sync_filter_results(
    ...
    if let Some(number) = number {
        ...
        for (item, _, indices) in ranked.iter() {
            println_json!(item.display, indices);
        }
    }
}
