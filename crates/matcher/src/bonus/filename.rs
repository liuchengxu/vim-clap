...
pub(crate) fn calc_bonus_filename(full_line: &str, score: Score, indices: &[usize]) -> Score {
    if let Some((_, idx)) = pattern::file_name_only(full_line) {
        if full_line.len() > idx {
            let hits_filename = indices.iter().filter(|x| **x >= idx).count();
            score * hits_filename as i64 / (full_line.len() - idx) as i64
        } else {
            0
        }
    } else {
