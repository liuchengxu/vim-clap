    ...
    O: Fn(&[S], usize) + Send + Sync + 'static + Copy,
{
    ...
    #[inline]
    pub fn lock_shared(&self) -> MutexGuard<MWPuple<S>> {
...
/// Recursive thread-creating function.
///
/// It will do the job as a one thread if called with `threadcount = 0`.
///
/// Creates bonus threads as long as the `threadcount != 0`,
/// doing `threadcount -= 1` after the bonus thread was spawned.
pub fn threader<I, P, E, F>(
    file_iter: &'static Mutex<I>,
    algo: &'static F,
    threadcount: u8,
) -> Vec<E>
where
    I: Iterator<Item = Result<P, E>> + FusedIterator + Send,
    P: AsRef<Path>,
    E: 'static + From<io::Error> + Send,
    F: Fn(Vec<u8>) + Sync,
{
    let mut unhandled_errors: Vec<E> = Vec::new();
    let mut handle_error = |e| unhandled_errors.push(e);

    const FBUF_LEN: usize = 4;

    let filebuffer: &mut [_; FBUF_LEN] = &mut {
        // Currently this: `[None; FBUF_LEN];` can't be done,
        // so here's that.
        [None, None, None, None]
    };
    fill_buf(filebuffer, file_iter.deref(), &mut handle_error);

    // If there's no more threadcount, we don't spawn bonus thread.
    // If there's no more items in iterator, we don't spawn bonus thread.
    let next_thread = if threadcount != 0 && filebuffer[FBUF_LEN - 1].is_some() {
        // let iter_ref_clone = Arc::clone(&file_iter);
        let iter_ref_clone = file_iter;

        let th = thread::spawn(move || threader(iter_ref_clone, algo, threadcount - 1));
        Some(th)
    } else {
        None
    };
    ...
    'outer: loop {
        for fbuf_idx in 0..FBUF_LEN {
            if let Some(file) = filebuffer[fbuf_idx].take() {
                match fs::read(file) {
                    Ok(bytevec) => algo(bytevec),

                    // That's either not a file or there's some error.
                    Err(e) => handle_error(E::from(e)),
                }
            } else {
                // There's no more work to do, end the loop.
                break 'outer;
    ...
    #[inline]
    fn fill_buf<I, P, E, F>(buf: &mut [Option<P>; FBUF_LEN], mutex: &Mutex<I>, handle_error: &mut F)
    where
        I: Iterator<Item = Result<P, E>> + FusedIterator,
        P: AsRef<Path>,
        E: From<io::Error>,
        F: FnMut(E),
    {
        let mut lock_iter = match mutex.lock() {
            Ok(v) => v,
            Err(pois) => pois.into_inner(),
        };
    ...
    if let Some(handle) = next_thread {
        let mut nthread_errors = match handle.join() {
            Ok(v) => v,
            Err(panic_data) => {
                handle_thread_panic(panic_data);

                vec![E::from(io::Error::new(
                    std::io::ErrorKind::Other,
                    "One of the threads throwed panic.",
                ))]
            }
...
#[cold]
fn handle_thread_panic(panic_data: impl Debug) {
    eprintln!("One of the threads had thrown panic.\nThat should not had happen, so here's the data:\n{:?}", panic_data);
}
