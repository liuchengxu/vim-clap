use crate::stdio_server::input::{Autocmd, PluginEvent};
...
impl CursorWordHighlighter {
    ...
    async fn create_new_highlights(&mut self) -> Result<Option<WinHighlights>> {
        ...
        // Lines in view.
        let (winid, line_start, line_end) = self.vim.current_visual_line_range().await?;
...
impl ClapPlugin for CursorWordHighlighter {
    async fn handle_event(&mut self, event: PluginEvent) -> Result<()> {
        match event {
            PluginEvent::Autocmd(autocmd) => match autocmd {
